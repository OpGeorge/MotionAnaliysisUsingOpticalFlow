...
Mat ReturnFlowDense(const string& name, const Mat& gray, const Mat& flow, float minVel = 0.0, bool showImages = true) {
...
void circleValShowHistogramDir(const string& name, int* hist, const int  hist_cols, const int hist_height, bool showImages = true, Point maxVal) {
	...
	if (showImages)
	{
		...
		for (int x = 0; x < hist_cols; x++) {
			...
			line(imgHist, maxVal, p2, CV_RGB(0, 0, 0));
		}
		printf("\n\n Point maxVal is x: %d , y: %d\n\n", maxVal.x, maxVal.y);
...
void applyMorpOp(Mat binary, Mat* input,vector<vector<Point>>* contoursOut) {
	...
	erode(binary, binary, element1, Point(-1, -1), 10);

	Mat element2 = getStructuringElement(MORPH_RECT, Size(3, 3));
	dilate(binary, binary, element2, Point(-1, -1), 10);
	...
	for (int i = 0; i < (*input).rows; i++) {
		for (int j = 0; j < (*input).cols; j++) {
			...
			ushort colorToAdd = 500;
			
			if (binary.at<uchar>(i, j) == 1 && (*input).at<ushort>(i, j) == 500) {
				
				for (int k = 0; k < 8; k++) {
					int ny = i + Ny[k];
					int nx = j + Nx[k];

					if (ny >= 0 && ny < (*input).cols &&
						nx >= 0 && nx < (*input).rows &&
						binary.at<uchar>(ny, nx) == 1 &&
						(*input).at<ushort>(ny, nx) != 500) {

						colorToAdd = (*input).at<ushort>(ny, nx);
					
					}
				
				}

				(*input).at<ushort>(i, j) = colorToAdd;
